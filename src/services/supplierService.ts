...
export async function createSupplier(name: string, priority: number, isDefault: boolean): Promise<Supplier> {
  ...
  if (isDefault) {
    await supabase.from("suppliers").update({ is_default: false } as any).eq("is_default", true);
  }
  const { data, error } = await supabase
    .from("suppliers")
...
export async function searchSupplierItems(query: string, limit = 20): Promise<SupplierItem[]> {
  ...
  const { data, error } = await supabase
    .from("supplier_items")
    .select("*, suppliers!inner(name, priority, is_active)")
    .ilike("searchable_text", searchTerm)
    .eq("suppliers.is_active", true)
    .order("suppliers(priority)", { ascending: true })
  ...
  return ((data ?? []) as any[]).map((row) => ({
    ...
    supplier_priority: row.suppliers?.priority ?? 99,
  }));
}
